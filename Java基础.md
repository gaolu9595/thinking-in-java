...
45. Java数组与其他种类的容器之间的区别有效率、类型和保存基本类型的能力三方面。数组（简单的线性序列）是一种效率最高的存储和随机访问对象引用序列的方式，但是为这种速度所付出的代价就是数组对象的大小是固定不可变的。数组之所以优于采取泛型之前的容器，是因为我们可以创建一个数组去持有某种具体类型，即可以在编译期通过检查来防止插入错误类型和抽取不当类型。而采用了泛型之后的容器，也可以指定他们的持有对象类型了，并且有了自动包装机制。
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE3MzU4MTcwNDIsNjUxNjE5NTI4LC0xNj
gzOTMxMjAxLC0zOTA2OTU1LC0xNzkyNDk1ODU3LC0xMDIyMjk4
OTgwLC05MzQ5MTMwODcsMTUwNTk4NzA0OCwxMTQzNTk5NDExLC
0zMTM0Nzk2MzAsLTg0ODg3MzA1NSwxNzYwMjIxMDM1LC0xMTM4
MTM3NjUyLDE4OTM1MDQ2OTgsLTExNDgzMTMzNzYsMjc0NTM5Mz
U0LDEyMzMzNjYyMzcsLTI4NTM4Mzc4MywxMjAzODc0NzgwLDEy
MTQwNDcyOTZdfQ==
-->