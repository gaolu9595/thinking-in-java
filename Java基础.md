...
45. Java数组与其他种类的容器之间的区别有效率、类型和保存基本类型的能力三方面。数组（简单的线性序列）是一种效率最高的存储和随机访问对象引用序列的方式，但是为这种速度所付出的代价就是数组对象的大小是固定不可变的。数组之所以优于采取泛型之前的容器，是因为
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEzNzgxODMwNTAsNjUxNjE5NTI4LC0xNj
